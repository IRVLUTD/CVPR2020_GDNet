...
def model_based_top_down_grasp(points_base):
    ...
    # if height < 0.04:
    #     z_tip -= height / 2
    #     z_tip = min(z_tip, 0.75)
    # else:
    #     z_tip -= 2 * margin_height
    print("Z_TIP:", z_tip)
...
def model_free_top_down_grasp(camera_pose, mask_id, label, xyz_image, percent_filter=0.025):
    ...
    plt.plot(grasps_origin[0, 0], grasps_origin[0, 1], 'bo')
    plt.plot(grasps_origin[1, 0], grasps_origin[1, 1], 'bo')    
    plt.plot(grasps_origin[2, 0], grasps_origin[2, 1], 'go')
    plt.plot(grasps_origin[3, 0], grasps_origin[3, 1], 'go')    
    plt.plot(grasps_origin[4, 0], grasps_origin[4, 1], 'yo')
    plt.plot(grasps_origin[5, 0], grasps_origin[5, 1], 'yo')        
    plt.axis('equal')      
    plt.show()          
    '''
